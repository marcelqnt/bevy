...
// always sorted into the following order: point lights, spot lights, bar lights,
// reflection probes, irradiance volumes.
struct ClusterableObjectIndexRanges {
    ...
    // The offset of the index of the first bar light, which also terminates
    // the list of spot lights.
    first_bar_light_index_offset: u32,
    // The offset of the index of the first reflection probe, which also
    // terminates the list of bar lights.
...
fn unpack_clusterable_object_index_ranges(cluster_index: u32) -> ClusterableObjectIndexRanges {
    ...
    let bar_light_offset = spot_light_offset + offset_and_counts_a.z;
    let reflection_probe_offset = bar_light_offset + offset_and_counts_a.w;
